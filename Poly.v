...
Theorem rev_involutive : forall X : Type, forall l : list X,
  rev (rev l) = l.
Proof.
  intros. induction l. reflexivity. simpl. rewrite rev_app_distr. rewrite IHl.
  reflexivity.
Qed.
(** [] *)
...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  match l with
  | [] => ([], [])
  | (x, y) :: tl => let (lx, ly) := (split tl) in (x :: lx, y :: ly)
  end.
...
Theorem map_rev_lemma: forall (X Y : Type) (f : X -> Y) (l : list X) (x : X),
...
Proof.
  ...
  rewrite map_rev_lemma. reflexivity.
Qed.
(** [] *)
