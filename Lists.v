...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | h :: t => let rt := remove_all v t in if v =? h then rt else h :: rt
  end.
...
Fixpoint subset (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | nil => true
  | h1 :: t1 => if 0 <? (count h1 s2) then subset t1 (remove_one h1 s2) else false
  end.
...
Theorem app_nil_r : forall l : natlist,
  l ++ [] = l.
Proof.
  intros. induction l. reflexivity. simpl. replace (l ++ []) with l. reflexivity.
Qed.
