...
(*
  All problems are finished by YouSiki <yousiki@pku.edu.cn> alone. 2022-2-28
*)
...
(* Evaluate nandb on all possible inputs *)
...
Fixpoint wrong_plus (n : nat) (m : nat) : nat :=
  if (n <=? m) then
    match n with
    | O => m
    | S n' => (wrong_plus n' (S m))
    end
  else
    match m with
    | O => n
    | S m' => (wrong_plus (S n) m')
    end.
...
(* Let's test [incr] on some examples ^_^ *)
Compute (incr Z).
Compute (incr (incr Z)).
...
(* Let's test [inbin_to_natcr] on some examples ^_^ *)
