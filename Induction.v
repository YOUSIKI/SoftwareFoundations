...
(** **** Exercise: 1 star, standard, optional (destruct_induction)
  ...
  [destruct term] only destruct term into its construct funtions.
  [induction term] also introduces the hypothesis on smaller term.
*)
...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
    ...
    - Next, suppose [n = S n'], where
      ...
      Rewrite using plus_n_Sm, this follows from
...
Theorem mul_m_Sn: forall m n: nat,
  m * (S n) = m + m * n.
Proof.
  intros m n.
  ...
  - simpl. rewrite add_assoc.
    ...
    rewrite H. rewrite IHm'. rewrite add_assoc. reflexivity.
Qed.
...
Proof.
  intros m n.
  induction m as [| m' IHm'].
  - assert (H1: n * 0 = 0). { induction n as [| n' IHn']. reflexivity. simpl. rewrite IHn'. reflexivity. }
    rewrite H1. reflexivity.
  - assert (H2: n * S m' = n + n * m'). { rewrite mul_m_Sn. reflexivity. }
    rewrite H2. simpl. rewrite IHm'. reflexivity.
Qed.
...
Proof.
  intros. induction p.
  rewrite mul_comm. simpl. rewrite mul_comm. simpl. rewrite mul_comm. simpl. reflexivity.
  rewrite mul_m_Sn. rewrite mul_m_Sn. rewrite mul_m_Sn. rewrite IHp.
  assert (H1: forall a b c:nat, a + b + c = a + (b + c)).
  intros. rewrite add_comm. rewrite <- add_shuffle3.
  assert (H2: c + b = b + c). rewrite add_comm. reflexivity. rewrite H2. reflexivity.
  rewrite <- H1. rewrite <- H1.
  assert (H3: n + m + n * p = n + n * p + m).
  rewrite H1.
  assert (H4: m + n * p = n * p + m). rewrite add_comm. reflexivity.
...
Proof.
  ...
  - simpl. rewrite add_0_r. rewrite add_0_r. rewrite IHn. simpl. rewrite add_comm. simpl. reflexivity.
Qed.
...
(*
  Because the definition of Type bin makes ( ...(B0 Z)) valid binary numbers.
  Actually, these binary numbers start with leading zeros, 
  which our definition of [nat_to_bin] and [bin_to_nat] doesn't take into account.
  Thus, [(nat_to_bin (bin_to_nat m))] will remove the leading zeros, returning another [bin].
*)
...
Fixpoint normalize (b:bin) : bin :=
  match b with
  | Z => Z
  | B0 b' => if iszero b' then Z else B0 (normalize b')
  | B1 b' => B1 (normalize b')
  end.
